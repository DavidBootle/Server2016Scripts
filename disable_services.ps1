...
if ($crit_services -notcontains "FTP") {
    try {
        Disable-WindowsOptionalFeature -Online -FeatureName IIS-FTPServer | Out-Null # disable the feature
...
if ($crit_services -notcontains "RDP") {
    try {
        ...
        Disable-WindowsOptionalFeature -Online -FeatureName Remote-Desktop-Services | Out-Null #turns off the feature if enabled
